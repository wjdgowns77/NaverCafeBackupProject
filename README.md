...
생성과 운영이 쉬워 우후죽순 생겨난 커뮤니티 중에는 운영자가 불어난 카페의 몸집을 감당할만한 자격이 되지 않는 경우도 있습니다.       
...
-이미 생성된 html파일을 pdf로 변환하는 과정에서 절대경로가 지정되지 않고 상대경로가 지정되어있는 경우에 한해 사진 깨짐등의 문제가 발생하지만, 네이버카페의 경우 절대경로를 지정하고 있어 이 문제는 없습니다. 
-그러나 해당 프로그램을 사용하여 네이버카페만 크롤링 하는것이 아니므로 수정의 필요성이 있습니다. 따라서 최대한 빠른시일내에 *.jpeg형식으로 스크린샷을 찍어 저장하는 방식도 추가할 생각입니다. 
